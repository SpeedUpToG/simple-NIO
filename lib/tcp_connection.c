...
/**
 * Discriptor: master dispatcher 创建connection， 并完成将socket fd事件注册到poll/epoll上
 */
...
tcp_connection_new(int connected_fd, struct event_loop *eventLoop,
    ...
    //新建连接加入到sub-dispatcher中（sub-dispatcher epoll 描述符注册socket fd 事件）
...
void tcp_connection_shutdown(struct tcp_connection *tcpConnection) {
    if (shutdown(tcpConnection->channel->fd, SHUT_WR) < 0) {
        msgx("tcp_connection_shutdown failed, socket == %d", tcpConnection->channel->fd);
    }
}