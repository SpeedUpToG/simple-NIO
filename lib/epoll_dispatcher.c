...
int epoll_dispatch(struct event_loop *eventLoop, struct timeval *timeval) {
    ...
    msgx("epoll_wait wakeup <n value:%d>, %s", n, eventLoop->thread_name);
    for (i = 0; i < n; i++) {
        ...
        if (epollDispatcherData->events[i].events & EPOLLIN) {
            msgx("get message channel fd==%d for read, %s", epollDispatcherData->events[i].data.fd, eventLoop->thread_name);
        ...
        if (epollDispatcherData->events[i].events & EPOLLOUT) {
            msgx("get message channel fd==%d for write, %s", epollDispatcherData->events[i].data.fd,eventLoop->thread_name);
